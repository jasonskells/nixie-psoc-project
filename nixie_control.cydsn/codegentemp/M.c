...
#include "M.h"
...
#if defined(M__PC)
    #define M_SetP4PinDriveMode(shift, mode)  \
    do { \
        M_PC =   (M_PC & \
                                (uint32)(~(uint32)(M_DRIVE_MODE_IND_MASK << \
                                (M_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (M_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define M_SetP4PinDriveMode(shift, mode)  \
        do { \
            M_USBIO_CTRL_REG = (M_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(M_DRIVE_MODE_IND_MASK << \
                                    (M_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (M_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		M_SetP4PinDriveMode(M__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(M_DR & (uint8)(~M_MASK));
    drVal = (drVal | ((uint8)(value << M_SHIFT) & M_MASK));
    M_DR = (uint32)drVal;
}
...
uint8 M_Read(void)
{
    return (uint8)((M_PS & M_MASK) >> M_SHIFT);
}
...
{
    return (uint8)((M_DR & M_MASK) >> M_SHIFT);
}
...
void M_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  M_INTCFG & (uint32)(~(uint32)position);
    M_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(M_INTSTAT & M_MASK);
	M_INTSTAT = maskedStatus;
    return maskedStatus >> M_SHIFT;
}
