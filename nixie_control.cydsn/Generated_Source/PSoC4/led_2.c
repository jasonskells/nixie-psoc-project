...
#include "led_2.h"
    ...
    #define led_2_SetP4PinDriveMode(shift, mode)  \
    do { \
        led_2_PC =   (led_2_PC & \
                                (uint32)(~(uint32)(led_2_DRIVE_MODE_IND_MASK << \
                                (led_2_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (led_2_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define led_2_SetP4PinDriveMode(shift, mode)  \
        do { \
            led_2_USBIO_CTRL_REG = (led_2_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(led_2_DRIVE_MODE_IND_MASK << \
                                    (led_2_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (led_2_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		led_2_SetP4PinDriveMode(led_2__0__SHIFT, mode);
    }
#endif
...
void led_2_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  led_2_INTCFG & (uint32)(~(uint32)position);
    led_2_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
